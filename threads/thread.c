...
tid_t thread_create(const char* name, int priority, thread_func* function, void* aux)
{
    ...
    /* Add to run queue. */
    thread_unblock(t); // 만든 쓰레드 t를 준비상태로 바꾸고 ready list에 리스트업 해줌
    if(thread_current()->priority < priority) // 현재 쓰레드와 생성한 쓰레드의 우선순위 비교하여 바로 cpu 양보
    thread_yield();
    return tid;
}
...
void thread_set_priority(int new_priority)
{
    ...
    struct thread* t = thread_current();
    struct thread* r = list_entry(list_begin(&ready_list), struct thread, elem);
