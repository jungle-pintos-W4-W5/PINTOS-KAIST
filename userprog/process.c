...
tid_t process_create_initd(const char* file_name)
{
    ...
    first_name = strtok_r(temp, " ", &save_ptr);
    ASSERT(!(first_name == NULL));

    /* Create a new thread to execute FILE_NAME. */
    tid = thread_create(first_name, PRI_DEFAULT, initd, fn_copy);
...
void process_exit(void)
{
    ...
    // --- [FD 테이블 정리 로직 시작] ---
    struct list_elem *e;
    // FD 테이블을 순회하며 모든 열린 파일들을 닫습니다.
    while (!list_empty(&curr->fd_table)) {
        e = list_pop_front(&curr->fd_table);
        struct fd_ *fd_entry = list_entry(e, struct fd_, elem);

        lock_acquire(&syslock); // 파일 시스템 접근 시 lock 사용
...
static bool load(const char* file_name, struct intr_frame* if_)
{
    ...
    strlcpy(fn_copy, file_name, MAX_CMLINE);

    if(!parse_argument(fn_copy, &argc, argv))
        goto done;
    ...
    set_argument_stack(if_, argc, argv);
...
void set_argument_stack(struct intr_frame* if_, int argc, char** argv)
{
    for(int i = argc - 1; i >= 0; i--){
        ...
        if_->rsp -= argv_len;
        memcpy((void*)if_->rsp, argv[i], argv_len);
        argv[i] = (char*)if_->rsp;
    }
